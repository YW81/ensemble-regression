...
addpath '..' % for pcrstar_choose_cutoff
...
%     y_rho_oracle = Ey + Z0' * w_rho_oracle;
    ...
    [y_sm, w_sm, t_max] = ER_SpectralApproach(Z,Ey,Ey2);
...
%     t_scan = linspace(-max(2,2*t_max),max(2,2*t_max),400);  % [0-max] 200
...
writetable(out, ['s-tmp-' hostname '.csv'],'WriteRowNames',true)
