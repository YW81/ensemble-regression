...
function [y_lrm, w_lrm, Cstar_offdiag, rho, v, rho_true, v_true, fval] = ER_Rank1MisfitNonNegResiduals( Z, Ey, Ey2 )
    [m,n] = size(Z);
...
%     [x,fval] = fminunc(cost,x0,options2);
    ...
    function [offdiag_residuals] = offdiag_elements(rho,v)
        ...
        offdiag_residuals(offdiag_residuals<0) = min(1/eps,exp(abs(offdiag_residuals(offdiag_residuals<0)))-1); % exponential penalty for negative residuals
    end
    ...
    R=zeros(m); R(idxs)=residuals./Ey2; R=R+R'; imagesc(R); colormap(hot); colorbar;title('Rank-1nn residuals'); 
    fprintf('non neg rank-1 norm(residuals) = %.2f',norm(R));
