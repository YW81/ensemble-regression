...
function [y_lrm, w_lrm,rho_hat] = ER_IndependentMisfitsNonNegResiduals( Z, Ey, Ey2 , loss)
    ...
    elseif strcmp(loss,'l1')
        ...
        [rho, resnorm, residuals] = lsqlin(A,Cshifted,-A,-Cshifted);%,[],[],zeros(m,1),Ey2*ones(m,1)); % require all residuals to be positive
        R=zeros(m); R(idxs)=residuals./Ey2; R=R+R'; imagesc(R); colormap(hot); colorbar;title('non-neg residuals'); 
        fprintf('non-neg norm(residuals) = %.2f',norm(R));
    end;
