...
%load UACC812.mat';
ROOT = './Datasets/RealWorld/Erhan/reheterogeneousensemblepaper/dream8_';
files = [struct('name','BT20.mat'), ...
         struct('name','BT549.mat'), ...
         struct('name','MCF7.mat'), ...
         struct('name','UACC812.mat')];
%files = struct(struct('name','UACC812.mat'));
...
if ~exist('MSE_orig','var')
    MSE_orig = zeros(m,length(files)); %to access use: sub2ind(tests_size,var_idx,cls_idx)
...
for i=1:m
    ...
    if strcmp(files(file_idx).name, 'UACC812.mat')
        print -depsc 'plots/icml/dream8_outlier_UACC812.eps';
        fprintf(2,'SAVED PLOT 141 TO plots/icml/dream8_outlier_UACC812.eps\n');
    end
...
%%
    ...
    results,
    fprintf(2,'BEST MODEL: %d\n',find(w_best));
    fprintf('PAUSED\n'); pause;    
end; % FOR FILES
%save dream8_results.mat