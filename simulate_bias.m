...
bias_list=-100:100;%[-1000 -100 -10 0 10 50 100 1000];
results = cell(numel(bias_list),1);
for b_itr=1:numel(bias_list)
    ...
    for iteration=1:10
        ...
        min_bias = bias_list(b_itr) -.75*(max(y_true) - min(y_true)); % -75
        max_bias = bias_list(b_itr) +1.5*(max(y_true) - min(y_true));  %+150
...
%     figure('Name',['max var=' num2str(vars_list(b_itr))]); 
%     subplot(121);
%     %set(gca,'fontsize',24);
    results{b_itr} = [res_oracle' ...
                      ...%res_oracle2' 
                      res_supervised' res_pcGEM' res_2me' res_uncentered_gem' ...
                      res_gem' res_uncorr' res_mean_f_i', res_f_best'];
%     boxplot(log10(results{b_itr}), ...
